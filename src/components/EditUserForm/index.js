  ...
  */
    ...
    return (
        (user ? <Formik
            ...
            initialValues={{ name: user.name, lastName: user.lastName, role: user.role }}
        >
            <Form className="container d-flex flex-column">
                ...
                {(isAdmin && <div className="d-flex flex-column spacing">
                    <label className="text-left">Role:</label>
                    <Field as="select" name="role">
                        {roles.map((role, key) => {
                            return (<option key={key} value={role.value}>{role.name}</option>)
                        })}
